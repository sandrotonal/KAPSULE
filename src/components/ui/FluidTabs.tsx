...
function FluidTabsList({
  ...
  return (
    <nav
      ...
      className={cn("relative overflow-visible rounded-full bg-surface border border-border p-1 shadow-md", className)}
      {...props}
    >
      <div
        role="tablist"
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "relative z-10 flex flex-1 items-center justify-center px-3 py-2 text-xs font-medium text-secondary hover:text-primary transition-colors",
    ...
    >
      {isSelected ? (
        <motion.span
          ...
          className="absolute inset-0 block rounded-full bg-accent/15 border border-accent/20 shadow-sm"
